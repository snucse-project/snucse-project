...
class BPlusTree {
  ...
  insert(key, username, start, end) {
    ...
    if (this.root === null) {
      ...
      if (this.root.numKeys === 2 * this.degree - 1) {
        ...
        newRoot.values[i].insertNonFull(key, newValue);
  ...
  visualize() {
    ...
    while (queue.length > 0) {
      ...
      if(!node.isLeaf){
        node.values.filter(value => value !== null).forEach(value => {
          result += `- ${value.keys.map(key => key === null ? 'null' : key).join(', ')} : `;
          result += value.numKeys + '\n';
          if (!value.isLeaf) {
            queue.push(value);
          }
        });
...
class BPlusTreeNode {
  constructor(degree, isLeaf) {
    ...
    this.values = new Array(2 * degree).fill(null);
    this.numKeys = 0;
    this.prev = null;
    this.next = null;
  }
  ...
  search(key) {
    ...
    return this.values[i].search(key);
  }
  ...
  insertNonFull(key, newValue) {
    ...
    if (this.isLeaf) {
      ...
      if (this.values[i + 1].numKeys === 2 * this.degree - 1) {
        this.splitChild(i + 1, this.values[i + 1]);
        if (this.keys[i + 1] < key) {
          i++;
        }
      }

      this.values[i + 1].insertNonFull(key, newValue);
    }
  }
  ...
  splitChild(i, y) {
    ...
    /*
    if(!y.isleaf){
      z.value[this.degree - 1] = y.value[2 * this.degree - 1];
      y.value[2 * this.degree - 1] = null;
    }*/
